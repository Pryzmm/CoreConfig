 ...
 * <p>
 * <code>COMMON</code> Configuration is stored on both the client and server and is synchronized between them. This can be changed on the clients end, but server settings will take priority over client settings. This is ideal for settings that can be customized by the player but should be overridden by the server for consistency.
*/
public enum ConfigType { CLIENT, SERVER, COMMON }